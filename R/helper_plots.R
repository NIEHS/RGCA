...
plot_sample_indv <- function(df) {
  ...
  ggplot2::ggplot(data = gg_df, aes(Concentration, Response, colour = as.factor(idx))) +
    geom_point() +
    geom_line() +
...
make_ggplot_many <- function(Cx_axis_values, resp_y_values, curve_data_list) {
  ...
  gg_df$Replicate <- 1
  hill_pred_plot <- ggplot2::ggplot(gg_df, aes(Cx, resp_y)) +
    geom_point(shape = 21, colour = "black", fill = "white", size = 0, stroke = 1) +
    scale_x_continuous(trans = "log10") +
  ...
  for (curve_idx in 2:num_curves) {
    ...
    hill_pred_plot <- hill_pred_plot +
      ...
      geom_ribbon(
        data = gg_df, aes(ymin = low, ymax = up, fill = ID),
        alpha = 0.1, # fill = "red",
        color = "black", linetype = "dotted"
      )
  }
...
plot_dummy_mixture <- function(Cx, y_i, tot_par_list, replicate_sets,
  ...
  curve_list <- predict_mix_response_many(
    n_dose = n_dose, chem_conc_matr = dummy_conc_mat,
    bootstrap_calc_list = bootstrap_calc_list,
    default_entry = NA
  )
  fit_resp <- hill_function(
    tot_par_list$sill_params[test_idx],
    tot_par_list$ec50_params[test_idx],
    tot_par_list$slope_params[test_idx],
    Cx[test_idx, ]
  )
...
plot_mixture_response <- function(plot_set, mix_df, mix_conc_df, mix_guide,
  ...
  for (iter_idx in 1:length(plot_set)) {
    ...
    annotated_plot <- myplot +
      ...
      ylab("Response") + labs(
        title = mix_df$CAS[mix_idx],
        subtitle = paste(strwrap(CAS_desc, width = .7 * getOption("width")), collapse = "\n")
      )
...
plot_mix_vs_individuals <- function(mix_idx = 34, ymax = 120) {
  ...
  for (i in active_idx) {
    ...
    matplot(
      x = t(Cx[matching_pure, ]), y = t(y_i_unscaled),
...
visualize_clusters_blocks <- function(slopes, clust_centers_w_prob, ul = 2, ll=0) {
  ...
  for (clust_id in 1:length(clust_names)) {
    ...
    for (cid in unique(clust_assign)) {
      points(
        x = slopes, y = rep(n_clust - clust_id, length(slopes)), xlim = c(min(slopes), 2),
...
plot_scores <- function(score_df, bootstrap_calc_list, method_levels = NA) {
  ...
  mdf <- rbind(
    ...
    cbind(reshape2::melt(subdf[, grep("MSE", names(score_df))]), "Score" = "MSE"),
...
plot_reflection_illustration <- function(slope_in = .5) {
  ...
  gplot <- ggplot2::ggplot(data = hill_df, aes(x = x, y = y, color = Segment)) +
    ...
    geom_rect(
      data = rect_region, inherit.aes = FALSE,
...
plot_inverse_illustration <- function(plot_reciprocal = F) {
  ...
  hilly_inverse_test <- function(y, c) hill_invs_factry(a, b, c)(y)
  # y_test_old = seq(abs(a)+.02, max_R-.01, by=.001)
  y_test <- seq(a + .04, 2 * a - .01, by = sign(a) * .01) #  [a, 2a]
  ...
  gplot <- ggplot2::ggplot(data = hill_df, aes(x = x, y = y, color = Segment)) +
    ...
    guides(colour = guide_legend(
      nrow = 1,
      override.aes = list(linetype = c(3, 1, 1, 1, 1))
    )) +
    ylab("Response") +
...
compare_exclude_include <- function() {
  plot_compare <- function(idx_set_list, conc_factor = 1, plt_title = NA) {
    ...
    p <- ggplot2::ggplot(plot_df, aes(x = CONC0, y = RESP0, group = group, color = group, shape = ReplicateSet)) +
