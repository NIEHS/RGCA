...
if (generate_data) {
  plot(x, hill_function(1, 1, 1, x) + 1.96 * .05,
       type = "l", col = 2, log = "x")
  points(x, hill_function(1, 1, 1, x))
  points(x, hill_function(1, 1, 1, x) - 1.96 * .05, type = "l", col = 2)
  for (i in seq(-.5, .5, by = .05)) {
    points(x, hill_function(1, 1, 1 + i, x), type = "l")
  }
  for (i in seq(-.5, .5, by = .05)) {
    points(x, hill_function(1, 1 + i, 1, x), type = "l")
  }
  ...
  n_dose <- length(Cx)
  # assume 3-4 clusters
  theta_1_true <- rnorm(n_chems, mean = 1, sd = .3)
