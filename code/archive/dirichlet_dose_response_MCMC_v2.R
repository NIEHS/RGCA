...
Cx = do.call(rbind, rep(list(Cx), n_chems))
...
for(iter in 1:n_iter){
  ...
  for(j in 1:n){
    ...
    for(k in 1:n_phis){
      ...
      pred_response = mk_vec(theta_1[j]/(1+(theta_2[j]/Cx[j,])^potential_phs[k]))
  ...
  for(j in 1:n_phi){
    ...
    for(itern in 1:num_sampling_iters){
      ...
      for(clm in cluster_member_idx){
        ...
        curr_mean = theta_1[j]/(1+(theta_2[j]/Cx[clm,])^phi_curr)
        llh_curr = sum(dnorm( y_i[clm,], mean = curr_mean, sd = sigma,log = T), 
                       na.rm = T)
        total_llh_curr = total_llh_curr + llh_curr
        
        prop_mean = theta_1[j]/(1+(theta_2[j]/Cx[clm,])^prop_phi_cj)
  ...
  for( j in 1:n){
    theta1_coeff = mk_vec(1/(1+(theta_2[j]/Cx[j,])^phi_c[c_i[j]]))
    theta1_post_var = 1/(sigma^(-2)*sum(theta1_coeff*theta1_coeff, na.rm = T)+2)
    XY_term = na.omit(theta1_coeff)%*%na.omit(mk_vec(y_i[j,]))
    theta_post_mean = theta1_post_var*(0+XY_term*sigma^(-2))
  ...
  for( j in 1:n){
    ...
    for(itern in 1:num_sampling_iters){
      ...
      curr_mean = mk_vec(theta_1[j]/(1+(theta_2[j]/Cx[j,])^phi_c[c_i[j]]))
      llh_curr = sum(dnorm( mk_vec(y_i[j,]), mean = curr_mean, sd = sigma,log = T), 
                     na.rm = T)+
      ...
      prop_mean = mk_vec(theta_1[j]/(1+(theta2_prop/Cx[j,])^phi_c[c_i[j]]))
      llh_prop = sum(dnorm( mk_vec(y_i[j,]), mean = prop_mean, sd = sigma,log = T), 
                     na.rm = T)+ 
  ...
  # update sigma
  #TODO allow each chemical to have a sigma
  p = length(phi_c)
  tot_error = 0
  for(j in 1:n){
    sum_sqr_err = sum((y_i[j,]-theta_1[j]/(1+(theta_2[j]/mk_vec(Cx[j,]))^phi_c[c_i[j]]))^2, 
                      na.rm = T)
