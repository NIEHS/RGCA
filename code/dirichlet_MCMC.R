...
DP_MCMC_fit = function(y_in, n_iter = 10000, sigma_in = .1){
  ...
  var_y_i = var(y_i)
  n_clust_expect = ceiling(log(length(y_i)))
  #n_iter = 10000
  ...
  for(iter in 1:n_iter){
    ...
    # update sigma
    p = length(phi_c)
    sigma_sqr = 1/rgamma(1, shape = 10 + n/2, 
  ...
  return(list("means"=record_assigned_mean,
...
cluster_centers = function(cluster_chain, n_top = 5, plot_hist = T){
  ...
  return(list("centers" = clust_centers,
              "center_sd" = clust_sds,
              "probs" = cluster_prob, 
...
if(F){
  ...
  plot(record_alpha); mean(record_alpha)
  plot(record_sigma); mean(record_sigma)
