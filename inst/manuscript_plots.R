...
make_ggplot <- function(Cx_axis_values,
  ...
  hill_pred_plot <- ggplot2::ggplot(gg_df, aes(Cx, resp_y)) +
    ...
    ggplot2::geom_line(aes(Cx, med, colour = "RE+DP")) +
    ggplot2::geom_ribbon(aes(ymin = low, ymax = up, fill = "RE+DP"),
                         alpha = 0.1,
  ...
  if (!is.null(curve_data_GCA)) {
    ...
    hill_pred_plot <- hill_pred_plot +
      ggplot2::geom_line(data = GCA_df,
                         aes(Cx, GCA_med, colour = "GCA")) +
      ggplot2::geom_ribbon(
      ...
      ggplot2::scale_colour_manual(name = "Legend",
                                   values = c("RE+DP" = "black",
                                              "GCA" = "red")) +
      ggplot2::scale_fill_manual(name = "Legend",
  ...
  if (!is.null(curve_data_IA)) {
    ...
    hill_pred_plot <- hill_pred_plot +
      ggplot2::geom_line(data = IA_df, aes(Cx, IA_med, colour = "IA")) +
      ggplot2::geom_ribbon(
...
make_ggplot_many <- function(Cx_axis_values, resp_y_values, curve_data_list) {
  ...
  gg_df$Replicate <- 1
  hill_pred_plot <- ggplot2::ggplot(data = gg_df, aes(Cx, resp_y)) +
    ggplot2::geom_point(shape = 21, colour = "black",
                        fill = "white",
                        size = 0,
                        stroke = 1) +
    ggplot2::scale_x_continuous(trans = "log10") +
    ggplot2::geom_line(aes(Cx, med, colour = ID)) +
  ...
  for (curve_idx in 2:num_curves) {
    ...
    names(gg_df) <- quantile_df_colnames
    hill_pred_plot <- hill_pred_plot +
      ggplot2::geom_line(data = gg_df, aes(Cx, med, colour = ID)) +
  ...
  hill_pred_plot <- hill_pred_plot +
    ...
    ggplot2::scale_fill_manual(name = "Legend", values = fill_vals)
...
plot_mixture_response <- function(plot_set, mix_df, mix_conc_df, mix_guide,
  ...
  for (iter_idx in seq(length(plot_set))) {
    ...
    myplot <- myplot +
      ggplot2::geom_point(data = unplt_mdf, size = 2,
                          aes(x = Cx_axis_values,
                              y = value,
                              shape = Replicate)) +
    ...
    fake_dats <- datasets::mtcars[1:18, 1:2]
    ...
    fake_plot <- ggplot2::ggplot(data = fake_dats, aes(mpg, cyl, color = col)) +
      ...
      ggplot2::scale_color_manual(name = "Active CAS", values = chem_cols)
    guide_color <- cowplot::get_legend(fake_plot)
    # combine the response plot with the legend from the arbitrary plot using
    # command plot_grid
    print(cowplot::plot_grid(annotated_plot + theme(legend.position = "right"),
                             guide_color,
                             rel_widths = c(.6, .2)))
  }
  return(score_record)
