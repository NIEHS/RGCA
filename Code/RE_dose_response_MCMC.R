...
if(generate_data){
  ...
  for(i in 1:n_chems){
    for(r in 1:n_replicates[n_chems]){
      ...
      print(curr_idx)
      response_data = hill_function(theta_1_true[i] + u_RE_true[curr_idx], theta_2_true[i],
                                    phi_ci_true[i],Cx)+v_RE_true[curr_idx] + rnorm(n_dose, sd=.05) 
  ...
  par(mfrow=c(3,3))
  for(i in 1:9){plot(y_i[i,-1]); points(y_i[i+n_chems,-1], pch=2); points(y_i[i+n_chems+1,-1],pch=3)}
  par(mfrow=c(1,1))
...
build_replicate_matrix = function(v_list){
  ...
  V_mat = matrix(0, nrow = num_repls*num_samples, 
...
# theta_2 = EC50
  ...
  for(iter in 1:n_iter){
    ...
    for( j in 1:n_chems){
      ...
      for(itern in 1:num_sampling_iters){
        ...
        llh_curr = sum(dnorm(active_Y, mean = curr_mean, sd = sigma[j],log = T), 
                       na.rm = T)+ 
          log(dlnorm(theta2_prop, meanlog = log(theta_2[j]), 
    ...
    tot_error =  0#theta_2[j]^2/sill_var[j]
      ...
      sigma_sqr = 1/rgamma(1, shape = .1+n_dose*length(act_idx)/2, 
                           rate = .1 + tot_error/2)
  ...
  return(list("slope_record" = record_assigned_mean,
...
if(F){
  ...
  points(Cx[idx1, ], hill_function(theta1_est[pi_idx],theta2_est[pi_idx], 
  ...
  cbind(c(v_RE_true[re_idx],theta_1_true[pidx],u_RE_true[re_idx], theta_2_true[pidx], phi_ci_true[pidx]),
        c(v_RE_sort[re_idx],theta_1[pidx],u_RE_sort[re_idx], theta_2[pidx], phi_c[c_i[pidx]]))
  
  plot(Cx[pidx,], v_RE_true[re_idx]+ hill_function(theta_1_true[pidx]+u_RE_true[re_idx], theta_2_true[pidx], 
                                                   phi_ci_true[pidx], Cx[pidx,]),
       ylim = c(0,2), log="x",type = "l", lty = 2)
  points(Cx[pidx,], y_i[re_idx,])
  lines(Cx[pidx,], v_RE_sort[re_idx] + hill_function(theta_1[pidx]+u_RE_sort[re_idx], 
  ...
  for( re_number in 2:length(replicate_sets[[pidx]])){
    ...
    lines(Cx[pidx,], v_RE_true[re_idx]+ hill_function(theta_1_true[pidx]+u_RE_true[re_idx], theta_2_true[pidx], 
                                                      phi_ci_true[pidx], Cx[pidx,]), col=re_number, lty = 2)
    # plot fitted curve
    lines(Cx[pidx,], v_RE_sort[re_idx] + hill_function(theta_1[pidx]+u_RE_sort[re_idx], 
                                                       theta_2[pidx], phi_c[c_i[pidx]], Cx[pidx,]),
          col=re_number)
  ...
  lines(Cx[re_idx,], v_RE_sort[re_idx] + hill_function(theta_1[pidx]+u_RE_sort[re_idx], 
  ...
  for( re_idx in replicate_sets[[pidx]][-1]){
    ...
    # plot fitted curve
    lines(Cx[re_idx,], 
          v_RE_sort[re_idx] + hill_function(theta_1[pidx]+u_RE_sort[re_idx], 
                                            theta_2[pidx], phi_c[c_i[pidx]], Cx[re_idx,]),
          col=clridx)
    clridx=clridx+1
